...
fpl<-data.frame("household_size" = c(1, 2, 3, 4, 5, 6, 7, 8),
           "poverty_guideline" = c(15650, 21150, 26650, 32150, 
                                   37650, 43150, 48650, 54150)
...
poverty_level <- function(x) {
  ...
  add_beyond_max <- 5500
  if (x > nrow(fpl)) {
    threshold <- fpl$poverty_guideline[nrow(fpl)]   + 
      (x - nrow(fpl)) * add_beyond_max
    
  } else{
    threshold <- fpl$poverty_guideline[which(fpl$household_size == x)]
  }
  return(threshold)
}
...
```{r}
#| code-fold: true
#| code-summary: "Show the code using paste"
# Your code here


makeURL<-function(household, year, state="US"){
  base_url<-'https://aspe.hhs.gov/topics/poverty-economic-mobility/poverty-guidelines/api'

  completed_url<-paste0(c(base_url, year, state, household), collapse="/")
...
```{r}
#| code-fold: true
#| code-summary: "Show the code using glue"



glueURL<-function(household, year, state="US"){
  library(glue)

  completed_url<-glue('https://aspe.hhs.gov/topics/poverty-economic-mobility/poverty-guidelines/api/{year}/{state}/{household}')
...
getFPL<-function(household, year,state="US"){
    base_url<-'https://aspe.hhs.gov/topics/poverty-economic-mobility/poverty-guidelines/api'
    completed_url<-paste0(c(base_url, year, state, household), collapse="/")
    # ... now send the request to the FPL site
    fpl_data <- jsonlite::fromJSON(completed_url)
    # and then get the income level from the resulting object
    income <- fpl_data$data$income
...
getFPL <- function(household, year, state = "US") {
  ...
  if(as.numeric(year)>current_year | as.numeric(year)<1983){
    stop("Invalid year entered")
  }
...
# use expand grid to get the combinations
fpl_table<-expand.grid(
            households = 1:4,
            years = 2020:2025
            )

# create an empty column to hold the FPL income
fpl_table$fpl<-NA

for(i in 1:nrow(fpl_table)){
  
  fpl_table$fpl[i]<-getFPL(fpl_table$households[i], fpl_table$years[i])
}
