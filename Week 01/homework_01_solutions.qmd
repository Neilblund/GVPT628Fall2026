...
```{r}
library(readr)
ches<-read_csv('https://github.com/chesdata/chesdata.github.io/releases/download/ches-europe/CHES_2024_final_v2.csv')
...
A. AKEL (The Progressive Party of Working People - Cyprus)

First, we need to use an aggregation to find the total number of seats in each country, then calculate the seat share for each party.

Here's an example of doing this using `group_by` and `mutate`:

```{r}

ches<-ches|>
  dplyr::group_by(country)|>
  dplyr::mutate(total_seats = sum(seat),
         seatshare = seat/total_seats
         )|>
  dplyr::ungroup()
...
Next, we need to sort the party name and country by another variable. We'll use a combination of subsetting along with `order`.
...
maxvalue<-order(radleft$seatshare, decreasing=T)[1]


radleft[maxvalue,c('party', 'country', 'seatshare')]
...
```{r}
# or use which.max()
maxvalue<-which.max(radleft$seatshare)
radleft[maxvalue,c('party', 'country', 'seatshare')]
...
ches|>
  ...
  dplyr::select(party, country, seatshare)|>
  dplyr::arrange(desc(seatshare))|>
  dplyr::slice_head(n=1)
